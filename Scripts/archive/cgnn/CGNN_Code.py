...
class CGNN_model(th.nn.Module):
    ...
    def run(self, data, train_epochs, test_epochs, lr, sample_size, kernel, saveData, index):
        ...
        for epoch in range(train_epochs):
            ...
            for data in train_loader:
                optim.zero_grad()
                mmd = MMD(self.forward()[:,0:4], data[:,0:4],kernel=kernel)
                mmd.backward()
                optim.step()
            ...
            if saveData:
                ...
                mmdList[epoch] = np.array([MMD(genData,simData,kernel=kernel).detach().numpy()]);
                simData = simData.numpy();
                genData = genData.detach().numpy();
                
                np.savetxt('./Debug/sim-' + str(index) + "-" + str(epoch) + ".dat",simData);
                np.savetxt('./Debug/gen-' + str(index) + "-" + str(epoch) + ".dat",genData);
        ...
        if saveData:
            np.savetxt('./Debug/mmd-' + str(index) + ".dat",mmdList);
        
        loss_abs, loss_rel=[], []
        for epoch in range(test_epochs):
            self.eval()
            for data in test_loader:
                mmd_abs  = MMD(data[:,0:4], self.forward()[:,0:4],kernel=kernel).item()
                mmd_base = np.mean([MMD(data[:,0:4], _data[:,0:4],kernel=kernel).item() for indx, _data in enumerate(test_loader) if indx<30])
                #mmd_base=1
...
class CGNN:
    def __init__(self, nh, batch_size, train_epochs, test_epochs, sample_size, patience=10, nruns=1, lr=0.01):
        ...
        self.patience=patience
        self.index = -1;
        self.enableDataSaving = True; #DEBUG
    ...
    def create_graph_from_data(self, data, given_candidates_path, saving_path, do_pruning=False, return_weights=False):
        ...
        for idx, cand in enumerate(cands):
            ...
            scores = CGNN.parallel_graph_evaluation(self, data =data , adj_matrix= candidate, tryDataSaving=True)
            ...
            if do_pruning:
                ...
                row[4+nb_vars**2:4+2**nb_vars**2]=cand_p10
            else:
                row[0]=idx
                row[1]=score_abs
                row[2]=score_rel
    ...
    def parallel_graph_evaluation(self, data, adj_matrix, tryDataSaving=False):
        ...
        for run in range(self.nruns):
            ...
            scores = model.run(data=data, train_epochs=self.train_epochs, test_epochs=self.test_epochs, lr=self.lr, sample_size=self.sample_size, kernel="multiscale" ,saveData=(tryDataSaving and self.enableDataSaving), index=self.index)
...
# Debug
obj_main = CGNN(nh=20, batch_size=1000, train_epochs=30, test_epochs=1, sample_size=10000)
