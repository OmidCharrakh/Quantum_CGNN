...
class MMD_Fourier(dist_base_class):
    ...
    '''
    def __init__(self, bandwidth = [0.01, 0.1, 1, 10, 100], n_RandComps=100):
        super().__init__('MMD_Fr', is_Fourier=True)
        self.scales = th.Tensor(bandwidth)
...
class NpMom(dist_base_class):
    ...
    '''
    def __init__(self, num_moments = 4, weighting_exp = 0.5):
        super().__init__('NpMom')
        self.num_std_moments = num_moments
...
class CndD(dist_base_class):
    ...
    '''
    def __init__(self, sample_weighting = False, sampling_rate = 4, num_std_moments = 4, weighting_exp = 0.5):
        super().__init__('CndD')
    ...
    def _calc_distr_diff_1(self, d1 , d2, binned_data_inds1, binned_data_inds2, list_var, list_cnd):
        ...
        return th.sum(diffs * weights) / th.sum(weights)
...
def _calc_moments(d, num_std_moments):
    ...
    for j in range(num_std_moments-2):
        moms = th.mean((d - means)**(j+3)) / stds**(j+3)
        if go_even:
