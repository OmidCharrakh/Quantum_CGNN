...
class CGNN_model(th.nn.Module):
    ...
    def forward(self, gen_size, only_observables=True):
        ...
        for n in self.topological_order:
            ...
            if n not in [0, 1]:
                parent_data = [
                    gen_data[p] for p in self.parents_ids[n]]
            else:
                parent_data = [
                    gen_data[{0:2,1:3}[n]], generate_scc(gen_data)]
...
class CGNN:
    def __init__(self, arc_nb_base_hu, arc_nb_layers, arc_wg_conn, lr, patience, loss_sum_pow, criteria_train, criteria_train_wgs, data_paths, data_paths_calib, results_path, mode_sgl_backprop, show_cand_plot, evaluate_training, nb_cpus=8):
        ...
        for folder in ['synthetic', 'profile', 'meta', 'losses/train', 'losses/valid', 'losses/test']:
...
def generate_scc(gen_data):
    data_settings = th.cat([gen_data[2], gen_data[3]], axis=1)
    ids_00 = th.logical_and((data_settings[:,0]<0.5), (data_settings[:,1]<0.5)).nonzero(as_tuple=True)[0]
    ids_01 = th.logical_and((data_settings[:,0]<0.5), (data_settings[:,1]>0.5)).nonzero(as_tuple=True)[0]
    ids_10 = th.logical_and((data_settings[:,0]>0.5), (data_settings[:,1]<0.5)).nonzero(as_tuple=True)[0]
    ids_11 = th.logical_and((data_settings[:,0]>0.5), (data_settings[:,1]>0.5)).nonzero(as_tuple=True)[0]
    data_scc = data_settings - data_settings
    data_scc[ids_00] = gen_data[4][ids_00]
    data_scc[ids_01] = gen_data[5][ids_01]
    data_scc[ids_10] = gen_data[6][ids_10]
    data_scc[ids_11] = gen_data[7][ids_11]
    return data_scc
